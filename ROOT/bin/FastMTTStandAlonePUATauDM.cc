...
void readdir(TDirectory *dir, optutl::CommandLineParser parser, char TreeToUse[], int recoilType, int doES, int isWJets, int metType, double tesSize) 
{
  ...
  while ((key = (TKey*)next())) {
	...
	return;
      ...
      t->SetBranchAddress("metcov11",&pfCovMatrix11);
      // Met Unc
      t->SetBranchAddress("pfmetEnUp",&uncMetPtUp);
      t->SetBranchAddress("pfmetEnDown",&uncMetPtDown);
      t->SetBranchAddress("pfmetphiEnUp",&uncMetPhiUp);
      t->SetBranchAddress("pfmetphiEnDown",&uncMetPhiDown);
      t->SetBranchAddress("pfmtEnUp",&clusteredMetPtUp);
      t->SetBranchAddress("pfmtEnDown",&clusteredMetPtDown);
      t->SetBranchAddress("pfmetphiJetUp",&clusteredMetPhiUp);
      t->SetBranchAddress("pfmetphiJetDown",&clusteredMetPhiDown);
      ...
      for(Int_t i=0;i<nevents;++i){
          t->GetEntry(i);
	  std::cout<<"Run: "<<run<<" lumi: "<<lumi<<" evt: "<<evt<<std::endl;
	  ...
	  tau2_up = tau2;
        ...
        if(doES) {
         ...
         float shiftTau1=1.0;
         float shiftTau2=1.0;
         if (gen_match_1==5 && decayMode==0) shiftTau1=1.007;
         if (gen_match_1==5 && decayMode==1) shiftTau1=0.998;
         if (gen_match_1==5 && decayMode==10) shiftTau1=1.001;
         if (gen_match_2==5 && decayMode2==0) shiftTau2=1.007;
         if (gen_match_2==5 && decayMode2==1) shiftTau2=0.998;
         if (gen_match_2==5 && decayMode2==10) shiftTau2=1.001;

         if ((gen_match_1==1 || gen_match_1==3) && decayMode==0)  shiftTau1=1.003;
         if ((gen_match_1==1 || gen_match_1==3) && decayMode==1)  shiftTau1=1.036;
         if ((gen_match_1==1 || gen_match_1==3) && decayMode==10) shiftTau1=1.000;
         if ((gen_match_2==1 || gen_match_2==3) && decayMode2==0)  shiftTau2=1.003;
	 if ((gen_match_2==1 || gen_match_2==3) && decayMode2==1)  shiftTau2=1.036;
	 if ((gen_match_2==1 || gen_match_2==3) && decayMode2==10) shiftTau2=1.000;
...
void CopyDir(TDirectory *source, optutl::CommandLineParser parser) {
  ...
  if(source->GetName()!=parser.stringValue("inputFile")){
    ...
    std::cout<<"Source name is not outputfile name it is "<<source->GetName() <<std::endl;
    adir->cd();    
  }
  else{
  ...
  std::cout<<"in adir: "<<source->GetName()<<std::endl;
  ...
  while ((key = (TKey*)nextkey())) {
    ...
    if (cl->InheritsFrom(TDirectory::Class())) {
      ...
      TDirectory *subdir = gDirectory;
      adir->cd();
      std::cout<<"copying directory "<< key->GetName()<<std::endl;
      CopyDir(subdir,parser);
      adir->cd();
    } else if (cl->InheritsFrom(TTree::Class())) {
      TTree *T = (TTree*)source->Get(key->GetName());
      std::cout<<"copying tree "<< key->GetName()<<std::endl;
      adir->cd();
      TTree *newT = T->CloneTree(-1,"fast");
      newT->Write();
      std::cout<<"wrote tree "<< key->GetName()<<std::endl;
    } else {
      source->cd();
      TObject *obj = key->ReadObj();
      std::cout<<"copying object "<< key->GetName()<<std::endl;
      adir->cd();
      obj->Write();
      std::cout<<"wrote object "<< key->GetName()<<std::endl;
      delete obj;
    }
  }
  adir->SaveSelf(kTRUE);
  savdir->cd();
  std::cout<<"finished copying directory"<<std::endl;
}
void CopyFile(const char *fname, optutl::CommandLineParser parser) {
...
void copyFiles( optutl::CommandLineParser parser, TFile* fOld, TFile* fNew) 
{
  ...
  //prepare files to be copied
  //Access PathName Returns FALSE if one can access a file using the specified access mode.
  // this is a bizarre convention (https://root.cern.ch/doc/master/classTSystem.html#a849c28ea0dd3b3aa3310a4d447c7b21a)
  ...
  if(gSystem->AccessPathName(parser.stringValue("inputFile").c_str())) {
    ...
    std::cout<<"cannot access path name creating new file "<<std::endl;
  }  
  else
    std::cout<<"can access Path Name"<<std::endl;

  fNew->cd();
  std::cout<<"cd'ed to "<<fNew->GetName()<<std::endl;
